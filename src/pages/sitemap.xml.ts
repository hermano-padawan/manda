...
export async function GET({ site }: { site: URL | undefined }) {
  ...
  const resources = await getCollection('resources', ({ data }) => data.type === 'mandala');
  const paths = new Set<string>(['/', '/mandalas/']);
  categories.forEach((category) => {
    paths.add(`/mandalas/${category.slug}/`);
    category.subcategories?.forEach((subcategory) => paths.add(`/mandalas/${category.slug}/${subcategory.slug}/`));
  });
  difficulties.forEach((item) => paths.add(`/mandalas/${item.slug}/`));
  audiences.forEach((item) => paths.add(`/mandalas/${item.slug}/`));
  resources.forEach((resource) => paths.add(`/mandalas/${resource.data.slug}/`));
